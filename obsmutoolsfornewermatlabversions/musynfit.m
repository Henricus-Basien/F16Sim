...
function [dsysl,dsysr] = musynfit(osysl,magdata,sens,blk,nmeas,ncntrl,...
                                    clpg,upbd,wt)
% clpg has the Previous rational D's
% magdata is the new incremental D's
% the returned dmdi is

if nargin < 6 | nargin == 7
 disp('usage: [dsysl,dsysr] = musynfit(pre_dsysl,Ddata,sens,blk,nmeas,ncntrl)')
 return
end
...
leftdim = nmeas;
 ...
 if nargin == 8
   if ~indvcmp(clpg,magdata)
    error('Closed-loop freq response & magdata have different IVs')
    return
   end
 end
...
%  all of the D-scalings by the inverse of the last D scaling.
 ...
 for i=1:nblk-1
   ...
   if nargin == 6
     [scalard] = fitmag(newdata,wt,heading,osysl_g_loc);
   else
     [scalard] = fitmag(newdata,wt,heading,osysl_g_loc,dmdi,upbd,blk,i);
   end
   tmpsys = [];
