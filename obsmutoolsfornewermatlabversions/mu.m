...
if mtype == 'syst'
        error('   mu for SYSTEM matrices is not implemented yet')
end
if  (nargin < 2)        % a default
    if mrows==mcols
        ...
        warning('   usage:  [bnds,rowd,sens,rowp,rowg] = mu(matin,blk,opt)')
        warning('   Nonsquare matin must have blk specified')
        return
    end % if mrows
end %if nargin
...
if any(opt=='C')
    ...
    warning(['mu Warning: MEX functions are unavailable.  Changing option from ' oldopt ' to ' opt]);
    end
    opt = [opt 'c'];
end
...
if ~sum(Ic+Jc)
    if ~any(opt=='w')
        warning('CAUTION this is a pure-real mu problem: If there are')
        warning(' convergence difficulties add some complex uncertainty.,')
    end
    ...
    if any(opt=='C') & ~nogo
        ...
        if diagnose==1
            if beta>bnds(1,2)
                ...
                warning([num2str(beta) ' ' num2str(newu) ' ' num2str(bnds(1,2))])
                warning(['% Improvement in Gap: ' num2str(100-100*percimp)]);
                warning(['Time to compute: ' num2str(etime(x2,x1))]);
            end
        end
    end
    nrmbigm = norm(M);
...
% GJB 13Aug08, added another "." due to error
            [tbnds,prt,dc,dri,dcb,drib,dcd,drid,g,y,b] = ...
rub(M,K,I,J,opt,numits,contol,Kc,Kr,Ic,Ir,Jc,Jr,...
...
rub(M,K,I,J,opt,numits,contol,Kc,Kr,Ic,Ir,Jc,Jr,...
    Fc,Fr,kc,kr,ic,ir,jc,jr,fc,fr,sc,sr,csc,csr,csF,...
    nK,nI,nJ,nF,nc,nr,kcK,icI,jcJ,jrJ,fcF,dcb,drib,...
    y,b,dc,dri,g,tbnds(1));
    else
            [tbnds,prt,dc,dri,dcb,drib,dcd,drid,g,y,b] = ...
rub(M,K,I,J,opt,numits,contol,Kc,Kr,Ic,Ir,Jc,Jr,...
    Fc,Fr,kc,kr,ic,ir,jc,jr,fc,fr,sc,sr,csc,csr,csF,...
